#!/bin/bash
# Fine-tune Gemma 4 31B IT (4-bit) for MedExpert Oncology
# Ejecutar en M4 (64GB RAM, Apple Silicon)
#
# Uso:
#   bash 06_finetune_gemma4.sh              # Run completo (3000 iters)
#   bash 06_finetune_gemma4.sh --pilot      # Run piloto (500 iters)
#
# Para ejecutar en background:
#   nohup bash 06_finetune_gemma4.sh > /dev/null 2>&1 &

set -e

PROJECT_DIR="/path/to/medexpert-oncologia"
DATA_DIR="$PROJECT_DIR/data/clean"
ADAPTERS_DIR="$PROJECT_DIR/adapters"
MLX_ENV="/path/to/mlx-env/bin"
...
if [ "$1" = "--pilot" ]; then
    ...
    SAVE_EVERY=100
    STEPS_PER_EVAL=100
    ADAPTER_SUFFIX="gemma4-31b-onco-pilot"
else
    echo "=== FULL TRAINING (3000 iters) ==="
    ITERS=3000
    SAVE_EVERY=500
    STEPS_PER_EVAL=300
    ADAPTER_SUFFIX="gemma4-31b-onco"
fi
...
$MLX_ENV/python -m mlx_lm lora \
    ...
    --grad-checkpoint \
    --mask-prompt \
    --config "$PROJECT_DIR/lora-config-gemma4.yaml" \
    $RESUME_FLAG \
    2>&1 | tee "$LOG_FILE"
