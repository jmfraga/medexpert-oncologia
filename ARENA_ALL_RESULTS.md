...
> **Note.** This file is a historical, consolidated narrative across all five evaluation rounds (v1, v2, v2b, Final, Matrix). The **authoritative per-judgment data** used in the manuscript is the raw CSV `supplementary_data/Supplementary_Data_2_arena_raw_480.csv` (480 judgments), and the headline base-vs-RAG-vs-fine-tuning contrasts use the single-round **Matrix** factorial with formal inference (`arena_within_round_inference.py`). Some early rounds included exploratory API baselines (e.g., ChatGPT) that are **not** part of the final within-round factorial and are shown here for historical completeness only.
